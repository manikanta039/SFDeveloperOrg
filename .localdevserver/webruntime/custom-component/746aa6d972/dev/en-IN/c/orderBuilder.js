Webruntime.define('lwc/orderBuilder', ['lwc', 'lightning/configProvider', 'c/orderItemTile', 'c/placeholder', 'c/inlineMessage', 'c/ldsUtils', 'wire-service', 'instrumentation/service', 'aura-storage', 'aura'], function (lwc, configProvider, _cOrderItemTile, _cPlaceholder, _cInlineMessage, ldsUtils, wireService, service, auraStorage, aura) { 'use strict';

    _cOrderItemTile = _cOrderItemTile && Object.prototype.hasOwnProperty.call(_cOrderItemTile, 'default') ? _cOrderItemTile['default'] : _cOrderItemTile;
    _cPlaceholder = _cPlaceholder && Object.prototype.hasOwnProperty.call(_cPlaceholder, 'default') ? _cPlaceholder['default'] : _cPlaceholder;
    _cInlineMessage = _cInlineMessage && Object.prototype.hasOwnProperty.call(_cInlineMessage, 'default') ? _cInlineMessage['default'] : _cInlineMessage;
    ...
    function stylesheet(hostSelector, shadowSelector, nativeShadow) {
      return "header" + shadowSelector + " {padding: 12px 16px;background-color: rgba(194, 51, 53, 1);color: #ffffff;border-top-left-radius: 0.25rem;border-top-right-radius: 0.25rem;display: flex;}\nheader" + shadowSelector + " .right" + shadowSelector + " {flex: 1;text-align: right;}\n.order-total" + shadowSelector + " {margin-left: 4px;}\n.drop-zone" + shadowSelector + " {background: rgb(243, 242, 242);display: flex;flex-wrap: wrap;border-bottom-left-radius: 0.25rem;border-bottom-right-radius: 0.25rem;}\nc-placeholder" + shadowSelector + ",c-order-item-tile" + shadowSelector + " {min-width: 250px;flex: 1;}\n";
    }
    ...
    **/
    ...
    function tmpl$1($api, $cmp, $slotset, $ctx) {
      ...
      return [api_element("div", {
        ...
        key: 8
      }, [$cmp.orderItems ? api_element("header", {
        classMap: {
        ...
        key: 1
      }, [api_text("Order Total:"), api_custom_element("lightning-formatted-number", _lightningFormattedNumber, {
        classMap: {
          "order-total": true
        },
        props: {
          "value": $cmp.orderPrice,
        ...
        key: 2
      }, [api_text("Total Items: "), api_dynamic($cmp.orderQuantity)])]) : null, $cmp.orderItems ? api_element("div", {
        classMap: {
          "drop-zone": true,
        ...
        key: 6,
        on: {
          "drop": _m2 || ($ctx._m2 = api_bind($cmp.handleDrop)),
          "dragover": _m3 || ($ctx._m3 = api_bind($cmp.handleDragOver))
        }
      }, api_flatten([api_iterator($cmp.orderItems, function (orderItem) {
        return api_custom_element("c-order-item-tile", _cOrderItemTile, {
          classMap: {
          ...
          key: api_key(4, orderItem.Id),
          on: {
            "orderitemchange": _m0 || ($ctx._m0 = api_bind($cmp.handleOrderItemChange)),
            "orderitemdelete": _m1 || ($ctx._m1 = api_bind($cmp.handleOrderItemDelete))
          }
        }, []);
      }), !$cmp.orderItems.length ? api_custom_element("c-placeholder", _cPlaceholder, {
        props: {
          "message": "Drag products here to add items to the order"
        },
        key: 5
      }, []) : null])) : null, $cmp.error ? api_custom_element("c-inline-message", _cInlineMessage, {
        props: {
          "errors": $cmp.error
        },
        key: 7
    ...
    tmpl$1.stylesheetTokens = {
      hostAttribute: "lwc-orderBuilder_orderBuilder-host",
      shadowAttribute: "lwc-orderBuilder_orderBuilder"
    };
    ...
    **/
     ...
     * Misc.
     */const refresh$1=bindWireRefresh(lds);const adsBridge=new AdsBridge(lds);const getRecordNotifyChange=throttle(60,60000,notifyChangeFactory(lds),{allowFunction:incrementGetRecordNotifyChangeAllowCount,dropFunction:incrementGetRecordNotifyChangeDropCount});/** UI API exports */const createRecord$2=createRecord$1;const deleteRecord$2=deleteRecord$1;const refresh$2=refresh$1;const updateRecord$2=updateRecord$1;/** Apex exports */const getApexInvoker$1=getApexInvoker;const getSObjectValue$1=getSObjectValue;// version: 0.1.10-3dfe453

    const apexInvoker = getApexInvoker$1("", "OrderController", "getOrderItems", false);

    var ORDER_ITEM_OBJECT = {"objectApiName":"Order_Item__c"};

    var ORDER_FIELD = {"fieldApiName":"Order__c","objectApiName":"Order_Item__c"};

    var PRODUCT_FIELD = {"fieldApiName":"Product__c","objectApiName":"Order_Item__c"};

    var QTY_SMALL_FIELD = {"fieldApiName":"Qty_S__c","objectApiName":"Order_Item__c"};

    var QTY_MEDIUM_FIELD = {"fieldApiName":"Qty_M__c","objectApiName":"Order_Item__c"};

    var QTY_LARGE_FIELD = {"fieldApiName":"Qty_L__c","objectApiName":"Order_Item__c"};

    var PRICE_FIELD = {"fieldApiName":"Price__c","objectApiName":"Order_Item__c"};

    var PRODUCT_MSRP_FIELD = {"fieldApiName":"MSRP__c","objectApiName":"Product__c"};
    ...
    function getQuantity(orderItem) {
      return getSObjectValue$1(orderItem, QTY_SMALL_FIELD) + getSObjectValue$1(orderItem, QTY_MEDIUM_FIELD) + getSObjectValue$1(orderItem, QTY_LARGE_FIELD);
    }
    /**
    ...
    function getPrice(orderItem, quantity) {
      return getSObjectValue$1(orderItem, PRICE_FIELD) * quantity;
    }
    /**
    ...
    class OrderBuilder extends lwc.LightningElement {
      constructor(...args) {
        super(...args);
      ...
      handleDrop(event) {
        event.preventDefault(); // Product__c from LDS

        const product = JSON.parse(event.dataTransfer.getData('product')); // build new Order_Item__c record

        const fields = {};
        fields[ORDER_FIELD.fieldApiName] = this.recordId;
        fields[PRODUCT_FIELD.fieldApiName] = product.Id;
        fields[PRICE_FIELD.fieldApiName] = Math.round(getSObjectValue$1(product, PRODUCT_MSRP_FIELD) * DISCOUNT); // create Order_Item__c record on server

        const recordInput = {
          apiName: ORDER_ITEM_OBJECT.objectApiName,
          fields
        };
        createRecord$2(recordInput).then(() => {
          // refresh the Order_Item__c SObjects
          return refresh$2(this.wiredOrderItems);
        }).catch(e => {
          this.dispatchEvent(new ShowToastEvent({
            title: 'Error creating order',
            message: ldsUtils.reduceErrors(e).join(', '),
            variant: 'error'
          }));
        });
      }
      ...
      handleOrderItemChange(evt) {
        const orderItemChanges = evt.detail; // optimistically make the change on the client
        ...
        this.setOrderItems(orderItems); // update Order_Item__c on the server

        const recordInput = {
          fields: orderItemChanges
        };
        updateRecord$2(recordInput).then(() => {// if there were triggers/etc that invalidate the Apex result then we'd refresh it
      ...
      handleOrderItemDelete(evt) {
        const id = evt.detail.id; // optimistically make the change on the client

        const previousOrderItems = this.orderItems;
        const orderItems = this.orderItems.filter(orderItem => orderItem.Id !== id);
        this.setOrderItems(orderItems); // delete Order_Item__c SObject on the server

        deleteRecord$2(id).then(() => {// if there were triggers/etc that invalidate the Apex result then we'd refresh it
          // return refreshApex(this.wiredOrderItems);
        }).catch(e => {
          // error updating server so rollback to previous data
          this.setOrderItems(previousOrderItems);
    ...
    lwc.registerDecorators(OrderBuilder, {
      publicProps: {
        recordId: {
          config: 0
        }
      ...
      fields: ["wiredOrderItems"]
    });

    var orderBuilder = lwc.registerComponent(OrderBuilder, {
      tmpl: _tmpl$1
    });
